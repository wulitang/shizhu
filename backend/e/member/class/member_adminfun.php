...
function admin_AddMember($add,$logininid,$loginin){
	global $empire,$dbtbpre;
	if(!trim($add[email])||!trim($add[username])||!$add[groupid]||!trim($add[password]))
	{
		printerror("未填写邮箱,用户名或者用户组","history.go(-1)",0,0,1);
	}
    CheckLevel($logininid,$loginin,$classid,"member");//验证权限
	//变量
	...
	$userfen=(int)$add[userfen];
	$userdate=(int)$userdate;
	$money=(float)$add[money];
	...
	if($num)
	{
		printerror('用户名重复',"history.go(-1)",0,0,1);
	}
	//注册时间
	$lasttime=time();
	...
	$sql=$empire->query("insert into ".eReturnMemberTable()."(".eReturnInsertMemberF('username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey').") values('$username','$password','$rnd','$email','$registertime','$groupid','$userfen','$userdate','$money','$add[zgroupid]','0','$checked','$salt','$userkey');");
	//取得userid
	...
	if($sql)
	{
	   insert_dolog("增加用户 userid=".$userid."<br>username=".$add[username]);//操作日志
	   printerror2("增加用户成功","AddMember.php?enews=AddMember".hReturnEcmsHashStrHref2(0));
	}
    else
	{
		printerror("DbError","history.go(-1)");
