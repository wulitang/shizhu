...
function AddFava($id,$classid,$cid,$from){
	...
	{	
        //增加每日收藏记录
	$sc=$empire->fetch1("select sctime,scnum from {$dbtbpre}enewsmemberadd where userid='$user_r[userid]'");
	$time=date("Y-m-d");
	if (!$sc[sctime] || $sc[sctime]!==$time){
		$empire->query("update {$dbtbpre}enewsmemberadd set scnum=1,sctime='$time',sclq='0' where userid='$user_r[userid]'");
	} else {
		$empire->query("update {$dbtbpre}enewsmemberadd set scnum=scnum+1 where userid='$user_r[userid]'");
	}
	//每日收藏记录结束
