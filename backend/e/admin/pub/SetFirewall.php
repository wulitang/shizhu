...
<body>
<div class="container">
<div class="nynav"><div>当前位置： <a href="main.php">后台首页</a> > <a href="SetFirewall.php<?=$ecms_hashur['whehref']?>">网站防火墙</a>  </div></div>
<div id="tab" style="margin-top:35px; padding-bottom:50px;">
<div class="ui-tab-bd">
<div class="ui-tab-content">
        	<h3><span>网站防火墙</span></h3>
            <div class="line"></div>
            <form name="setform" method="post" action="SetFirewall.php" onSubmit="return confirm('确认设置?');">
  <?=$ecms_hashur['form']?>
            <input name="enews" type="hidden" id="enews" value="SetFirewall"> 
			<ul>
            		<li class="jqui"><label>开启防火墙</label><input type="radio" name="fw_open" value="1"<?=$ecms_config['fw']['eopen']==1?' checked':''?>>
        开启 
        <input type="radio" name="fw_open" value="0"<?=$ecms_config['fw']['eopen']==0?' checked':''?>>
        关闭</li>
                    <li class="jqui"><label>防火墙加密密钥</label><input name="fw_pass" type="text" id="fw_pass" value="<?=$ecms_config['fw']['epass']?>" size="20">
		    <input type="button" name="Submit3" value="随机" onclick="document.setform.fw_pass.value='<?=make_password(36)?>';"><font color="#666666">(填写10~50个任意字符，最好多种字符组合)</font></li>
                    <li class="jqui"><label>允许后台登陆的域名</label><input name="fw_adminloginurl" type="text" id="fw_adminloginurl" value="<?=$ecms_config['fw']['adminloginurl']?>" size="35">
        <font color="#666666">
        (设置后必须通过这个域名才能访问后台，如：http://admin.yecha.com)</font></li>
                    <li class="jqui"><label>允许登陆后台的时间点<br> <font color="#666666">(不选为不限制)</font></label><table width="500" border="0" cellspacing="1" cellpadding="3">
          <tr> 
          ...
          <tr> 
            ...
            <td><input name="fw_adminhour[]" type="checkbox" id="fw_adminhour[]" value="23"<?=strstr(','.$ecms_config['fw']['adminhour'].',',',23,')?' checked':''?>>
              23点</td>
                    ...
                    <li class="jqui"><label>允许登陆后台的星期<br> <font color="#666666">(不选为不限制)</font></label><table width="500" border="0" cellspacing="1" cellpadding="3">
          <tr> 
          ...
          <tr> 
            ...
            <td>&nbsp;</td>
                    ...
                    <li class="jqui"><label>防火墙后台预登陆验证变量名</label><input name="fw_adminckpassvar" type="text" id="fw_pass3" value="<?=$ecms_config['fw']['adminckpassvar']?>" size="35">
        <font color="#666666">(由英文字母组成,5~20个字符组成)</font></li>
                    <li class="jqui"><label>防火墙后台预登陆认证码</label><input name="fw_adminckpassval" type="text" id="fw_adminckpassval" value="<?=$ecms_config['fw']['adminckpassval']?>" size="35">
        ...
        <li><label>屏蔽提交敏感字<br>
          ...
          设置屏蔽前台所有提交内容及后台登陆内容)</font></label><textarea name="fw_cleargettext" id="fw_cleargettext" class="textarea"><?=ehtmlspecialchars($ecms_config['fw']['cleargettext'])?></textarea></li>
            </ul>
