...
function CreateClassCache($bclassid,$exp,$expjs,$expmodjs,$adminclass,$doall,$mid,$addminfocid,$oldmid,$oldaddminfocid,$userid){
	...
	while($r=$empire->fetch($sql))
	{
		...
		if($r[islast])
		{
			$img="<a href='AddNews.php?<?=$ecms_hashur[ehref]?>&enews=AddNews&classid=".$r[classid]."' target=_blank><img src='../data/images/txt.gif' border=0></a>";
			$bgcolor="#ffffff";
			$renewshtml=" <a href='#e' onclick=renews(".$r[classid].",'".$r[tbname]."')>".$fun_r['news']."</a> ";
			$docinfo=" <a href='#e' onclick=docinfo(".$r[classid].") id='docinfo".$r[classid]."'>归档</a>";
		...
		$onelistclass="<tr bgcolor='".$bgcolor."' height=25><td><input type=text name=myorder[] value=".$r[myorder]." size=2><input type=hidden name=classid[] value=".$r[classid]."></td><td".$divonclick.">".$exp.$img."</td><td align=center>".$r[classid]."</td><td><input type=checkbox name=reclassid[] value=".$r[classid]."> <a href='".$classurl."' target=_blank id='classname".$r[classid]."'>".$classname."</a></td><td align=center>".$r[onclick]."</td><td><a href='javascript:editlm(".$r[classid].")'>".$fun_r['edit']."</a> <a href='javascript:copylm(".$r[classid].")'>".$fun_r['copyclass']."</a> <a href='javascript:dellm(".$r[classid].")' id='class".$r[classid]."'>".$fun_r['del']."</a></td><td><a href='#e' onclick=relist(".$r[classid].")>".$fun_r['re']."</a>".$renewshtml."<a href='#e' onclick=rejs(".$r[classid].")>JS</a> <a href='#e' onclick=tvurl(".$r[classid].")>调用</a>".$classinfotype.$docinfo."</td></tr>";
		...
		//------ 管理信息页面 ------
		//链接地址
		$infoclassurl=sys_ReturnBqClassUrl($r);
		//终级栏目
		if($r[islast])
		{
			//最后一个子栏目
			...
			{$menutype="file";}
			$infoclassname="<a showmenu='1' onclick='tourl($r[bclassid],$r[classid]);' menu='menu$r[classid]' classid='$r[classid]' bclassid='$r[bclassid]' classurl='$infoclassurl' onmouseout=\"this.style.fontWeight=''\" onmouseover=\"this.style.fontWeight='bold'\" class='menuarr'  title='栏目ID：$r[classid]'>".$r[classname]."</a>";
			$onmouseup="";
		}
		else
		{
			...
			$infoclassname="<a showmenu='2' menu='menu$r[classid]' classid='$r[classid]' bclassid='$r[bclassid]' classurl='$infoclassurl' class='menuarr'  title='栏目ID：$r[classid]'>".$r[classname]."</a>";
		}
		$returnr['listenews'].='<span id="pr'.$r[classid].'" class="'.$menutype.'" onclick="'.$onmouseup.'">'.$infoclassname.'</span>';
		//JS颜色
		if($r[islast])
		{
		...
		if($userid&&empty($doall))
		{
			if(CheckHaveInClassid($r,$adminclass))
			{
				...
				$returnr['userenews'].='<span id="pr'.$r[classid].'" class="'.$menutype.'" onclick="'.$onmouseup.'">'.$infoclassname.'</span>';
		...
		if(empty($r[islast]))
		{
			...
			if(empty($r['wburl']))
			{
			$returnr['listenews'].='<ul id="item'.$r[classid].'" style="display:none"><li class="'.$listtype.'">'.$retr['listenews'].'</li></ul>';
			if($havelevel)
			{
				$returnr['userenews'].='<ul id="item'.$r[classid].'" style="display:none"><li class="'.$listtype.'">'.$retr['userenews'].'</li></ul>';
