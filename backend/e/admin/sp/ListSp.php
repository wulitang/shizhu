...
function CheckAll(form)
  ...
  //增加碎片
function zjsp(){
art.dialog.open('sp/AddSp.php?<?=$ecms_hashur[ehref]?>&enews=AddSp',
    {title: '增加碎片',lock: true,opacity: 0.5, width: 800, height: 540,
	 close: function () {
      location.reload();
    }
	});
...
art.dialog.open('sp/ListSpClass.php<?=$ecms_hashur['whehref']?>',
    {title: '管理碎片分类',lock: true,opacity: 0.5, width: 800, height: 540,
	 close: function () {
      location.reload();
    }
	});
...
<div class="ui-tab-content">
        	<h3><span>选择分类： <select name="classid" id="classid" onchange=window.location='ListSp.php?cid='+this.options[this.selectedIndex].value>
        <option value="0">显示所有分类</option>
        <?=$scstr?>
      </select> <a href="javascript:void(0)" onclick="zjsp()" class="add">增加碎片</a> <a href="javascript:void(0)" onclick="glspfl()" class="add">管理碎片分类</a></span></h3>
            <div class="line"></div>
<div class="jqui">
...
<table class="comm-table anniuqun" cellspacing="0">
  ...
  ?>
    ...
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="8" style="text-align:left;"> 
        <?=$returnpage?><input type="submit" name="Submit" value="刷新碎片文件">
      <input name="enews" type="hidden" id="enews" value="ReSp">
      </td>
    </tr>
	</tbody>
</table>
