...
//增加标题分类
function zjbtfl(){
art.dialog.open('info/AddInfoType.php?<?=$ecms_hashur[ehref]?>&enews=AddInfoType',
    {title: '增加标题分类',lock: true,opacity: 0.5, width: 800, height: 540,
	close: function () {
      location.reload();
    }
	});
...
<body>
<div class="container">
<div class="nynav"><div>当前位置： <a href="main.php">后台首页</a> > <a href="InfoType.php<?=$ecms_hashur['whehref']?>">管理标题分类</a></div></div>
<div id="tab" style="margin-top:35px;">
<div class="ui-tab-bd">
<div class="ui-tab-content">
        	<h3><span>选择所属系统模型： <select name="mid" id="mid" onchange="window.location='InfoType.php?mid='+this.options[this.selectedIndex].value;">
        <option value="0">不限系统模型</option>
        <?=$mstr?>
      </select> <a href="javascript:void(0)" onclick="zjbtfl()" class="add">增加标题分类</a></span></h3>
            <div class="line"></div>
<div class="jqui">
  ...
  ?>
   <tr bgcolor="#FFFFFF"> 
      ...
      <td><a href="../ecmschtml.php?enews=ReTtHtml&typeid=<?=$r[typeid]?>&fmid=<?=$mid?><?=$ecms_hashur['href']?>">刷新</a> 
        ...
        <a href="#ecms" onclick="window.open('../view/TtUrl.php?ttid=<?=$r[typeid]?><?=$ecms_hashur['ehref']?>','','width=600,height=450');">调用</a></td>
    </tr>
    <?
  }
  ?>
