  ...
  ?>
	...
	<div class="ui-tab-bd">
		<div class="ui-tab-content">
			<h3><span>增加栏目</span></h3>
            <div class="line"></div>
            <ul>
            	<li><label>栏目名称</label><div class="jqui"><input name="classname" type="text" id="classname" value="<?=$r[classname]?>" size="20"></div>&nbsp;
            <?
	  if($enews=="AddClass")
	  {
	  ?>
            <input type="button" name="Submit5" value="生成拼音目录" onClick="window.open('GetPinyin.php?hz='+document.form1.classname.value+'&returnform=opener.document.form1.classpath.value<?=$ecms_hashur['href']?>','','width=160,height=100');" class="anniu"> 
	  ...
	  ?>
            ...
            <input name="filepass" type="hidden" id="filepass" value="<?=$filepass?>"></li>
                <li class="jqui"><label>栏目别名</label><input name="bname" type="text" id="bname" value="<?=$r[bname]?>" size="20"><font color="#666666">(为空则与栏目名相同)</font></li>
              ...
              <?=$options?>
                ...
                <li class="jqui"><label>栏目类型</label><?php
		  ...
		  ?>
            ...
            <i>外部栏目</i><font color="#666666">(选择后不能修改)</font> 
            <?php
			}
			?></li>
            	<div id="wbclass"<?=$wbclassstyle?>>
                <li class="jqui"><label>外部栏目链接地址：</label><input name="wburl" type="text" id="wburl" value="<?=$r[wburl]?>" size="20">
              <input name="oldwburl" type="hidden" id="oldwburl" value="<?=$r[wburl]?>"></li>
              	</div>
                <div id="nbclass"<?=$nbclassstyle?>>
                <li><label>是否终级栏目</label><?=$islast?>
              <font color="#FF0000">(终级栏目下才能增加信息)</font></li>
                <li><label>栏目存放文件夹</label><input name="oldcpath" type="hidden" id="oldcpath" value="<?=$classpath?>"><input name="oldclasspath" type="hidden" id="oldclasspath" value="<?=$r[classpath]?>"><table border="0" cellspacing="1" cellpadding="3" class=" comm-table2" style="width:400px;">
                ...
                <tr> 
                  ...
                  <td><input name="classpath" type="text" id="classpath3" value="<?=$classpath?>" size="15"<?=$read?>></td>
                  <td> <div align="left"> 
                      <input type="button" name="Submit3" value="检测目录" onClick="javascript:window.open('ecmscom.php?<?=$ecms_hashur[ehref]?>&enews=CheckPath&pripath='+document.form1.pripath.value+'&classpath='+document.form1.classpath.value,'','width=100,height=100,top=250,left=450');" class="anniu">
                    </div></td>
                </tr>
              </table></li>
                </div>
                <li><label>绑定的系统模型</label><select name="modid" id="modid">
                <?=$mod_options?>
              </select> <input type="button" name="Submit6" value="管理系统模型" onClick="glxtmx()" class="anniu">
              * 
              <input name="oldmodid" type="hidden" id="oldmodid" value="<?=$r[modid]?>"></li>
                <li><label>使用优化方案</label><select name="yhid" id="yhid">
				<option name="0">不使用</option>
                <?=$yh_options?>
              </select> 
              <input type="button" name="Submit63" value="管理优化方案" onClick="glyh()" class="anniu"></li>
                <li><label>绑定域名</label><div class="jqui"><input name="classurl" type="text" id="classurl" value="<?=$r[classurl]?>" size="20"></div>&nbsp;
              <input name="UrlToSmall" type="checkbox" id="UrlToSmall" value="1">
              <font color="#666666">应用于子栏目 (没有绑定，请留空.后面无需加&quot;/&quot;)</font></li>
				<li class="jqui"><label>栏目缩略图</label><input name="classimg" type="text" id="classimg" value="<?=$r[classimg]?>" size="20"> 
            <a onClick="window.open('ecmseditor/FileMain.php?modtype=1&type=1&classid=&doing=2&field=classimg&filepass=<?=$filepass?>&sinfo=1<?=$ecms_hashur['ehref']?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../data/images/changeimg.gif" border="0" align="absbottom"></a></li>
                <li class="jqui"><label>页面关键字</label><input name="classpagekey" type="text" id="classpagekey" value="<?=$r[classpagekey]?>" size="20"></li>
                <li class="jqui"><label>栏目简介</label><textarea name="intro" cols="70" rows="8" id="intro"><?=stripSlashes($r[intro])?></textarea></li>
        <?php
		if($enews=='EditClass')
		{
		?>
                <li class="jqui"><label>栏目增加时间</label><?=$r['addtime']?date("Y-m-d",$r['addtime']):'---'?></li>
        <?php
		}
		?>
             <div class="clear"></div>
		...
		<div class="ui-tab-content" style="display:none">
            ...
            <li class="jqui"><label>开启前台投稿</label> 
            ...
            <li class="jqui"><label>发布信息增加</label><input name="addinfofen" type="text" id="addinfofen2" value="<?=$r[addinfofen]?>" size="6">
              <i>点数</i> <font color="#666666">(不增加请设为0,扣点请设为负数，使用此项需将投稿权限设置为会员以上)</font></li>
            ...
            <li><label>信息预设投票</label><select name="definfovoteid" id="definfovoteid">
              ...
              <font color="#666666">(增加信息时默认的投票项)</font></li>
            <li><label>默认查看信息权限</label><select name="groupid" id="groupid">
		...
		<div class="ui-tab-content" style="display:none">
            ...
            <div class="line"></div>
                <ul>
                <div id="smallclassbdinfo">
                <li><label>栏目绑定信息</label><i>绑定信息ID：</i>
              <input name="smallbdinfoid" type="text" id="smallbdinfoid" value="<?=$r[bdinfoid]?>">
              <a href="#empirecms" onClick="EditSmallBdInfo(document.form1);">[修改信息]</a> <font color="#666666">(格式：栏目ID,信息ID)</font></li>
            	</div>
