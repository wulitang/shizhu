...
<form name="AddNewsForm" id="AddNewsForm" method="get">
        	<h3><span><div class="left">管理信息 <a href="ListAllInfo.php?tbname=<?=$tbname?><?=$ecms_hashur['ehref']?>" title="已发布信息总数：<?=$tbinfos?>" <?=$indexchecked==1?' class="glon"':'class="gl"'?> style=" margin-left:10px;">已发布 (<?=$tbinfos?>)</a> <a href="ListAllInfo.php?tbname=<?=$tbname?>&ecmscheck=1<?=$ecms_hashur['ehref']?>" <?=$indexchecked==0?' class="glon"':'class="gl"'?>>待审核 (<?=$tbckinfos?>)</a></div><div class="left" style="margin:0 10px;"><div id="showaddclassnav" style="margin-top:7px;"></div></div><a href="javascript:void(0)" onclick="if(document.AddNewsForm.addclassid.value!=0){window.open('AddNews.php?<?=$ecms_hashur['ehref']?>&enews=AddNews<?=$addecmscheck?>&classid='+document.AddNewsForm.addclassid.value,'','');}else{alert('请选择要增加信息的栏目');document.AddNewsForm.addclassid.focus();}" class="add">增加信息</a> <a href="ecmschtml.php?enews=ReIndex<?=$ecms_hashur['href']?>" class="sx">刷新首页</a> <a href="ecmschtml.php?enews=ReAllNewsJs&from=<?=$phpmyself?><?=$ecms_hashur['href']?>" class="sx">刷新所有信息JS</a></span></h3>
</form>
            <div class="line"></div>
           	<div class="saixuan">
...
<tr> 
      <td height="25" colspan="8"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            ...
            <td>&nbsp;&nbsp;<font color="#ffffff"><a href="ListAllInfo.php?tbname=<?=$tbname?><?=$addecmscheck?>&sear=1&showspecial=8<?=$ecms_hashur['ehref']?>">我的信息</a> 
                ...
                | <a href="ReHtml/ChangeData.php<?=$ecms_hashur['whehref']?>" target=_blank>更新数据</a> | <a href="../../" target=_blank>预览首页</a></font></td>
          </tr>
        </table></td>
    </tr>
    </table>
