...
function jianfan(){
	...
	var toFT=getCookie(JF_cn);
	if(toFT>0){$(".fanyi").html("繁体中文");} else {$(".fanyi").html("简体中文");};
  StranBody();
}
...
{
  var JF_cn="ft"+self.location.hostname.toString().replace(/\./g,"")
	var toFT=getCookie(JF_cn),zh;
  if(toFT>0){zh=1;} else {zh=0};
 ...
 {
  var OO=obj.item(i)
  if(OO.title!=""&&OO.title!=null)OO.title=StranText(OO.title,zh);
    if(OO.alt!=""&&OO.alt!=null)OO.alt=StranText(OO.alt,zh);
    if(OO.tagName=="INPUT"&&OO.value!=""&&OO.type!="text"&&OO.type!="hidden")OO.value=StranText(OO.value,zh);
    if(OO.nodeType==3){OO.data=StranText(OO.data,zh)}
  else StranBody(OO)
 }
}
